...
describe('過去のお届け先がない場合', () => {
  ...
  test('入力、送信すると入力内容が送信される', async () => {
    ...
    render(<Form2 onSubmit={onSubmit} />);
...
describe('過去のお届け先がある場合', () => {
  test('設問に答えるまで、お届け先を選べない', () => {
    render(<Form2 deliveryAddresses={deliveryAddresses} />);
    expect(
  ...
  test('いいえを選択し、入力、送信すると、入力内容が送信される。', async () => {
    const [mockFn, onSubmit] = mockHandleSubmit();

    expect.assertions(2);

    render(<Form2 deliveryAddresses={deliveryAddresses} onSubmit={onSubmit} />);
  ...
  test('はいを選択し、入力、送信すると、入力内容が送信される。', async () => {
    const [mockFn, onSubmit] = mockHandleSubmit();
    ...
    const contactNmuber = await inputContactNumber();
    const deliveryAddress = await inputDeliveryAddress();
    await clickSubmit();
    expect(mockFn).toHaveBeenCalledWith(
      expect.objectContaining({ ...contactNmuber, ...deliveryAddress })
    );
  });
});
